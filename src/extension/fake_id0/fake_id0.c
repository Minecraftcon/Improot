...
static int handle_sysexit_end(Tracee *tracee, Config *config)
{
	...
	case PR_getgroups32: {
		...
		int count = (config->num_groups > 0) ? config->num_groups : 1;
		...
		for (int i = 0; i < count; i++) {
			gid_t g = (config->num_groups > 0) ? config->groups[i] : config->rgid;
			if (sysnum == PR_getgroups32) {
				poke_uint32(tracee, list_addr + i * sizeof(uint32_t), g);
			} else {
				poke_uint16(tracee, list_addr + i * sizeof(uint16_t), (uint16_t)g);
			}
			if (errno != 0)
				return -errno;
		}
	...
	case PR_setgroups32: {
		...
		if (size < 0 || size > FAKE_MAX_GROUPS) {
			poke_reg(tracee, SYSARG_RESULT, -EINVAL);
			return 0;
		}

		for (int i = 0; i < size; i++) {
			if (sysnum == PR_setgroups32) {
				config->groups[i] = (gid_t) peek_uint32(tracee, list_addr + i * sizeof(uint32_t));
			} else {
				config->groups[i] = (gid_t) peek_uint16(tracee, list_addr + i * sizeof(uint16_t));
			}
			if (errno != 0)
				return -errno;
		}

		config->num_groups = size;
		poke_reg(tracee, SYSARG_RESULT, 0);
		return 0;
	}
