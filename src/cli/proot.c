...
static int handle_option_vdisk(Tracee *tracee, const Cli *cli UNUSED, const char *value)
{
	...
	const char *cache_dir = vdisk_get_cache_dir();
	if (cache_dir != NULL) {
		new_binding(tracee, cache_dir, "/", true);
		new_binding(tracee, "/dev", "/dev", false);
		new_binding(tracee, "/proc", "/proc", false);
		new_binding(tracee, "/sys", "/sys", false);
		new_binding(tracee, "/etc/resolv.conf", "/etc/resolv.conf", false);
		new_binding(tracee, "/etc/hosts", "/etc/hosts", false);

		/* Default working directory to /root inside guest */
		tracee->fs->cwd = talloc_strdup(tracee->fs, "/root");

		/* Set clean guest environment */
		setenv("USER", "root", 1);
...
static int handle_option_persistent(Tracee *tracee UNUSED, const Cli *cli UNUSED, const char *value UNUSED)
{
	vdisk_set_persistent(true);
	return 0;
}

static int handle_option_setup_paths(Tracee *tracee UNUSED, const Cli *cli UNUSED, const char *value)
{
	if (value && (strcmp(value, "off") == 0 || strcmp(value, "no") == 0 || strcmp(value, "0") == 0 || strcmp(value, "[/]") == 0 || strcmp(value, "/") == 0 || strcmp(value, "[ / ]") == 0)) {
		vdisk_set_setup_paths(false);
	} else {
			...
			size_t argc UNUSED, char *const argv[] UNUSED, size_t cursor)
{
	...
	/* Default to /root or / for vdisk if no CWD was specified, else "." */
	if (tracee->fs->cwd == NULL) {
		if (vdisk_get_cache_dir() != NULL) {
			status = handle_option_w(tracee, cli, "/root");
			if (status < 0)
	...
	const char *discovered_path = vdisk_get_discovered_path_env();
	if (discovered_path != NULL) {
		setenv("PATH", discovered_path, 1);
	}
